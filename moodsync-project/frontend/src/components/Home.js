...
const Home = ({ 
  ...
  const handleGeneratePlaylist = async (boardOrUrl) => {
    ...
    try {
      ...
      // Automatically generate playlist preview after analysis
      setTimeout(() => {
        handleAutoGeneratePreview(analysisData);
      }, 1000); // Small delay to ensure analysis is set
  ...
  const handleAutoGeneratePreview = async (analysisData) => {
    try {
      ...
      if (data.success && data.playlist) {
        // Store the preview data in localStorage or state for PlaylistCreator to access
        localStorage.setItem('autoGeneratedPreview', JSON.stringify(data.playlist));
        // Trigger a re-render to show the preview
        setAnalysis({...analysisData, autoPreview: data.playlist});
      }
